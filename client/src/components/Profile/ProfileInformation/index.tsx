...
const ProfileInformation: React.FC<props> = () => {
    ...
    const renderNavigate = useMemo(() => {
        return PROFILE_LINK.map((item, index) => {
            return <NavigateProfileItem title={item.title} key={index} to={item.to} active={pathname === item.to}>
                <Typography
            ...
            <Box className={styles.headerProfile}>
                <Box className={styles.backgroundInformation}>
                    <Box className={styles.blur}>
                        <img className={styles.coverBlur} src={userCurrent.userInfo.coverImage || coverImageDefault}
                             alt=""/>
                    </Box>
                    <Container className={styles.information}>
                        <Grid container>
                            <Grid item xs={10}>
                                <img className={styles.cover} src={userCurrent.userInfo.coverImage || coverImageDefault}
                                     alt=""/>
                                {self ? <ButtonBase
                                    id="basic-button"
                                ...
                                <Box className={styles.avatar}>
                                    <Avatar
                                        alt="Remy Sharp"
                                        src={userCurrent.userInfo.avatar || avatarDefault}
