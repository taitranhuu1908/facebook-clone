...
public class RoomService implements IBaseService<Room, Long> {
    ...
    public Room findByUsers(User user, User friend) {
        return this.findAll().stream().filter(room -> room.getUsers().contains(user) && room.getUsers().contains(friend)).findFirst().orElse(null);
    }
}
